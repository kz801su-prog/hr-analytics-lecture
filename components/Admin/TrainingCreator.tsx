...
import {
  generateQuestionsFromMaterials,
  regenerateQuestionFromMaterials,
...
export const TrainingCreator: React.FC<TrainingCreatorProps> = ({
  onUpdateTraining,
  trainings,
  results,
  wrongAnswerAnalyses,
  onSaveWrongAnswerAnalysis,
  onOpenSelectKey,
  employees,
}) => {
  ...
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("sb_training_flags") || "{}");
    } catch {
      return {};
    }
  });
  ...
  const handleSelectTraining = (t: Training) => {
    ...
    const flagsFromStorage: Record<
      string,
      { isRequiredForAll: boolean; fiscalYear?: number }
    > = JSON.parse(localStorage.getItem("sb_training_flags") || "{}");
    setIsRequiredForAll(
      flagsFromStorage[t.id]?.isRequiredForAll ?? t.isRequiredForAll ?? false,
    );
    setFiscalYear(
      flagsFromStorage[t.id]?.fiscalYear ??
        t.fiscalYear ??
    ...
    const pattern =
      t.patterns.find((p) => p.id === t.activePatternId) || t.patterns[0];
    if (pattern) {
  ...
  const handleSaveAllChanges = () => {
    ...
    if (editingQuestions.length === 0)
      return alert("保存する問題がありません。");

    const patternId =
      currentPatternId &&
      currentPatternId !== "AI_TEMP" &&
      currentPatternId !== "CSV_TEMP"
        ? currentPatternId
        : "PT-" + Math.random().toString(36).substring(2, 7).toUpperCase();
    ...
    if (editingTraining) {
      const updatedPatterns = editingTraining.patterns.map((p) =>
        p.id === patternId ? newPattern : p,
      );
    ...
    const newFlags = {
      ...trainingFlags,
      [updated.id]: { isRequiredForAll, studyLinks, fiscalYear },
    };
  ...
  const handleGenerate = async () => {
    ...
    const timer = setInterval(
      () => setGenStep((p) => (p < steps.length - 1 ? p + 1 : p)),
      2500,
    );
    try {
      const qs = await generateQuestionsFromMaterials(
        title,
        content,
        materials,
        questionCount,
  ...
  const handleRegenerateQuestion = async (qIdx: number) => {
    if (!title) return alert("研修タイトルを入力してください");
    const targetQuestion = editingQuestions[qIdx];
    if (!targetQuestion) return;

    setRegeneratingQuestionIndex(qIdx);
    try {
      const newQuestion = await regenerateQuestionFromMaterials(
        title,
        content,
        materials,
        targetQuestion,
      ...
      setEditingQuestions((prev) =>
        prev.map((q, idx) =>
          idx === qIdx ? { ...newQuestion, id: targetQuestion.id } : q,
        ),
      );
      alert(`Q${qIdx + 1}を再生成しました。内容を確認してください。`);
    } catch (e: any) {
      console.error("単問再生成エラー:", e);
      const errorMessage = e?.message || "AI再生成エラーが発生しました。";
      alert(
  ...
  const handleCsvImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (parsedQuestions.length > 0) {
        ...
        alert(
          `${parsedQuestions.length}件の問題を読み込みました。内容を確認して保存してください。`,
        );
      }
    };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const f of files) {
      try {
        const material = await new Promise<TrainingMaterial>(
          (resolve, reject) => {
            const reader = new FileReader();
    ...
    if (skippedFiles > 0) {
      alert(
        `${newMaterials.length}件のファイルを追加しました。\n${skippedFiles}件のファイルはスキップされました。`,
      );
    }
  ...
  const handleAnalyzeWrongAnswers = (training: Training) => {
    const trainingResults = results.filter(
      (r) => r.trainingId === training.id && r.postScore !== -1,
    );
    ...
    const analyses: WrongAnswerAnalysis[] = questions.map((q, qIdx) => {
      ...
      const wrongRate =
        totalAttempts > 0 ? Math.round((wrongCount / totalAttempts) * 100) : 0;
  ...
  const handleFetchFromUrl = async () => {
    ...
    try {
      ...
      if (!data.contents) {
        throw new Error(
          "コンテンツの取得に失敗しました。URLが公開されているか確認してください。",
        );
      }

      console.log(
        "✓ HTMLコンテンツを取得しました (サイズ:",
      ...
      if (!cleanedText || cleanedText.length < 10) {
        throw new Error(
          "有効なテキストコンテンツが見つかりませんでした。別の方法でコンテンツを取得してください。",
        );
      }
      ...
      try {
        ...
        alert(
          "テキストのエンコードに失敗しました。コンテンツフィールドには追加されています。",
        );
        setUrlInput("");
      }
    } catch (error) {
  ...
  const getTrainingFiscalYear = (t: Training) => {
    const explicitFY = trainingFlags[t.id]?.fiscalYear ?? t.fiscalYear;
    if (typeof explicitFY === "number" && !Number.isNaN(explicitFY)) {
      return explicitFY;
    }
  ...
  const trainingFiscalYear = (t: Training) => getTrainingFiscalYear(t);

  const availableYears = Array.from(
    new Set(
      trainings
        .map((t) => trainingFiscalYear(t))
  ...
  const filteredTrainings = trainings.filter((t) => {
    ...
    return trainingFiscalYear(t) === selectedFiscalYear;
  });
    ...
    <div className="space-y-6 md:space-y-8 animate-fadeIn pb-16 md:pb-20">
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="p-6 md:p-8 border-b bg-gradient-to-r from-rose-600 to-orange-600 text-white flex justify-between items-center">
              <div>
                ...
                <p className="text-xs md:text-sm font-bold opacity-90 mt-1">
                  多くの人が間違えた問題（誤答率 高→低順）
                </p>
              </div>
              <button
                onClick={() => setShowWrongAnswerModal(false)}
                className="w-10 h-10 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white font-black transition-all"
              >
                ✕
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
              {wrongAnswerData.map((analysis, idx) => (
                <div
                  key={idx}
                  className="p-6 md:p-8 bg-slate-50 rounded-2xl border-2 border-slate-100 space-y-4"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className="text-xs font-black text-white bg-rose-600 px-3 py-1 rounded-full">
                          Q{analysis.questionIndex + 1}
                        </span>
                        <span className="text-xs font-black text-rose-600 bg-rose-100 px-3 py-1 rounded-full">
                          誤答率 {analysis.wrongRate}%
                        </span>
                        <span className="text-[10px] font-bold text-slate-400">
                          {analysis.wrongCount}/{analysis.totalAttempts}人が誤答
                        </span>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {analysis.options.map((opt, oIdx) => {
                      ...
                      const wrongCount = analysis.wrongEmployees.filter(
                        (e) => e.selectedAnswer === oIdx,
                      ).length;
                      return (
                        <div
                          key={oIdx}
                          className={`p-3 rounded-xl border-2 flex items-center justify-between ${isCorrect ? "border-emerald-500 bg-emerald-50" : "border-slate-200 bg-white"}`}
                        >
                          <div className="flex items-center gap-2">
                            <span
                              className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black ${isCorrect ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"}`}
                            >
                              {String.fromCharCode(65 + oIdx)}
                            </span>
                            <span
                              className={`text-sm font-bold ${isCorrect ? "text-emerald-800" : "text-slate-600"}`}
                            >
                              {opt}
                            </span>
                          </div>
                          {wrongCount > 0 && !isCorrect && (
                            <span className="text-[10px] font-black text-rose-600 bg-rose-100 px-2 py-1 rounded">
                              {wrongCount}人
                            </span>
                          )}
                          {isCorrect && (
                            <span className="text-[10px] font-black text-emerald-600 bg-emerald-100 px-2 py-1 rounded">
                              正解
                            </span>
                          )}
                        </div>
                    ...
                    <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {analysis.wrongEmployees.map((emp, empIdx) => (
                          <span
                            key={empIdx}
                            className="text-xs font-bold text-amber-800 bg-amber-100 px-3 py-1 rounded-full"
                          >
                            {emp.employeeName}{" "}
                            <span className="text-[10px]">
                              (選択:{" "}
                              {String.fromCharCode(65 + emp.selectedAnswer)})
                            </span>
                          </span>
                        ))}
                      </div>
      ...
      <section className="bg-slate-900 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl text-white space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2 items-center bg-slate-800 rounded-xl p-1 border border-slate-700">
            <button
              onClick={() => setSelectedFiscalYear(currentFY)}
              className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${selectedFiscalYear === currentFY ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white"}`}
            >
              今期
            </button>
            <button
              onClick={() => setSelectedFiscalYear("all")}
              className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${selectedFiscalYear === "all" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white"}`}
            >
              通年
            </button>
            <div className="flex items-center gap-2 px-2 py-2 bg-slate-900 rounded-xl">
              ...
              <select
                ...
                className="px-3 py-2 rounded-xl border border-slate-700 bg-slate-800 text-white text-sm font-black outline-none"
              >
                <option value="all">通年表示</option>
                {availableYears.map((year) => (
                  <option key={year} value={year}>
                    {getTermFromFiscalYear(year)}年度 ({year}年4月〜)
                  </option>
                ))}
              </select>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 overflow-x-auto">
          {[...filteredTrainings]
            ...
            .map((t) => {
              const isAllRequired =
                trainingFlags[t.id]?.isRequiredForAll ??
                t.isRequiredForAll ??
                false;
              ...
              return (
                <div
                  key={t.id}
                  className={`p-4 md:p-5 rounded-xl md:rounded-2xl text-left transition-all border-2 flex flex-col justify-between h-32 md:h-36 cursor-pointer ${editingTraining?.id === t.id ? "bg-indigo-600 border-white shadow-lg" : "bg-slate-800 border-slate-700 hover:border-slate-500"}`}
                  onClick={() => handleSelectTraining(t)}
                >
                  <div className="space-y-1">
                    <div className="font-black text-xs md:text-sm truncate w-full">
                      {t.title}
                    </div>
                    <div className="flex gap-1 flex-wrap">
                      {isAllRequired && (
                        <span className="inline-block text-[8px] font-black bg-rose-600 text-white px-2 py-0.5 rounded-full">
                          🔴 全員必須
                        </span>
                      )}
                      <span className="inline-block text-[8px] font-black bg-indigo-500/30 text-indigo-200 px-2 py-0.5 rounded-full">
                        {getTermFromFiscalYear(tFY)}年度
                      </span>
                    </div>
                  </div>
          ...
          <button
            ...
            className="p-4 md:p-5 rounded-xl md:rounded-2xl border-2 border-dashed border-slate-700 text-slate-500 font-bold hover:text-white hover:border-indigo-500 flex items-center justify-center gap-2 transition-all h-28 md:h-32"
          >
            <span>＋</span> 新規作成
          </button>
        </div>
      ...
      <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] p-6 md:p-10 border shadow-sm space-y-8 md:space-y-10">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b pb-4 md:pb-6 gap-2">
          <h2 className="text-xl md:text-2xl font-black text-slate-800">
            {editingTraining ? "研修詳細の修正" : "新規研修の登録"}
          </h2>
          {editingTraining && (
            <span className="text-[10px] md:text-xs font-black text-indigo-600 bg-indigo-50 px-3 md:px-4 py-1 md:py-2 rounded-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
          <div className="space-y-2">
            ...
            <input
              type="text"
              placeholder="例: セキュリティ研修 2024"
              className="w-full px-4 md:px-6 py-3 md:py-4 rounded-xl border-2 font-bold focus:border-indigo-500 outline-none transition-all text-sm md:text-base"
              value={title}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              >
                {Array.from({ length: 5 }, (_, i) => currentFY - 2 + i).map(
                  (y) => (
                    <option key={y} value={y}>
                      {getTermFromFiscalYear(y)}年度 ({y}年4月〜)
                    </option>
                  ),
                )}
              </select>
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              className={`w-full px-4 py-3 md:py-4 rounded-xl border-2 font-black text-xs md:text-sm transition-all ${
                isRequiredForAll
            ...
            <div className="space-y-2 bg-slate-50 p-3 rounded-xl">
              {studyLinks.map((link, idx) => (
                <div key={idx} className="flex gap-2 items-center">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setStudyLinks(studyLinks.filter((_, i) => i !== idx))
                    }
              ...
              {studyLinks.length < 5 && (
                <button
                  type="button"
                  onClick={() =>
                    setStudyLinks([...studyLinks, { label: "", url: "" }])
                  }
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <span className="text-lg font-black text-indigo-600 w-8">
                {questionCount}
              </span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
            <div
              ...
              className="border-2 border-dashed border-slate-200 rounded-2xl md:rounded-3xl p-6 md:p-8 flex flex-col items-center justify-center gap-2 md:gap-3 cursor-pointer hover:border-indigo-500 hover:bg-indigo-50/30 transition-all group"
            >
              <svg
                className="w-8 h-8 md:w-10 md:h-10 text-slate-300 group-hover:text-indigo-500 transition-all"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <p className="text-xs md:text-sm font-bold text-slate-400 group-hover:text-indigo-600">
                ファイルを追加
              </p>
              <input
                type="file"
            ...
            <div className="space-y-2 max-h-[140px] md:max-h-[180px] overflow-y-auto pr-2 scrollbar-hide">
              {materials.length > 0 ? (
                materials.map((m, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 md:p-4 bg-slate-50 border border-slate-100 rounded-xl md:rounded-2xl animate-fadeIn"
                  >
                    <span className="text-[10px] md:text-xs font-bold text-slate-600 truncate max-w-[80%]">
                      {m.name}
                    </span>
                    <button
                ...
                <div className="h-full flex items-center justify-center text-slate-300 text-[10px] md:text-xs italic border-2 border-slate-50 rounded-2xl md:rounded-3xl p-6">
                  資料なし
                </div>
              )}
            </div>
        ...
        <div className="space-y-4 border-t pt-6">
          ...
          <p className="text-[9px] text-blue-600 font-bold">
            💡
            公開されているWebページやNotebookLMの共有URLを入力できます。取得に失敗する場合は、テキストエリアに直接ペーストしてください。
          </p>
        </div>

        {/* Actions */}
        <div className="p-6 md:p-8 bg-indigo-50 rounded-2xl md:rounded-3xl border-2 border-indigo-100 space-y-4 md:space-y-6">
          <p className="font-black text-indigo-800 italic text-center text-xs md:text-sm">
            モデル・難易度・問題数を選択してAI生成
          </p>
          <div className="flex flex-col gap-3 md:gap-4 max-w-2xl mx-auto">
            <div className="flex gap-2">
              {["gemini-2.5-flash", "gemini-3-flash-preview"].map((m) => (
                <button
                  ...
                  className={`flex-1 py-3 md:py-4 rounded-xl md:rounded-2xl font-black border-2 transition-all text-xs ${modelName === m ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-400 border-slate-200"}`}
                >
                  {m.includes("3-flash")
                    ? "Gemini 3 Flash"
                    : "Gemini 2.5 Flash"}
            ...
            <div className="flex gap-2">
              {(["Standard", "Difficult", "MAX"] as const).map((d) => (
                <button
                  ...
                  className={`flex-1 py-3 md:py-4 rounded-xl md:rounded-2xl font-black border-2 transition-all text-xs ${difficulty === d ? "bg-amber-500 text-white border-amber-500" : "bg-white text-slate-400 border-slate-200"}`}
                >
                  {d === "Standard"
                    ? "標準"
                    : d === "Difficult"
            ...
            <div className="flex gap-2">
              {[10, 20, 50].map((c) => (
                <button
                  ...
                  className={`flex-1 py-3 md:py-4 rounded-xl md:rounded-2xl font-black border-2 transition-all text-sm ${questionCount === c ? "bg-indigo-600 text-white border-indigo-600" : "bg-white text-slate-400 border-slate-200"}`}
                >
                  {c}問
                </button>
              ))}
            ...
            <button
              ...
              className="w-full py-4 bg-indigo-600 text-white rounded-xl md:rounded-2xl font-black text-sm md:text-base hover:bg-indigo-700 disabled:bg-indigo-200 shadow-lg"
            >
        ...
        <div className="space-y-6 md:space-y-10 animate-slideUp editing-area-marker">
          <div className="flex flex-col md:flex-row justify-between items-center bg-emerald-600 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] shadow-xl gap-4 md:gap-6">
            <div className="text-white text-center md:text-left">
              ...
              <p className="font-bold opacity-80 italic text-[10px] md:text-xs">
                最後に下の「保存」ボタンで確定してください。
              </p>
            </div>
            <button
              onClick={handleSaveAllChanges}
              className="w-full md:w-auto px-8 md:px-16 py-4 md:py-6 bg-white text-emerald-700 rounded-xl md:rounded-[2rem] font-black text-base md:text-xl shadow-xl"
            >
              すべて保存
            </button>
          </div>
          ...
          <div className="space-y-6 md:space-y-8">
            {editingQuestions.map((q, qIdx) => (
              <div
                key={qIdx}
                className="p-6 md:p-10 bg-white rounded-[2rem] md:rounded-[3rem] border-2 border-slate-100 shadow-sm space-y-6 md:space-y-8 hover:border-indigo-400 transition-all"
              >
                <div className="flex flex-col sm:flex-row gap-4 md:gap-8 justify-between items-start">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-slate-900 text-white flex items-center justify-center font-black text-xl md:text-2xl shrink-0 shadow-lg">
                      Q{qIdx + 1}
                    </div>
                    <div className="space-y-2">
                      ...
                      <textarea
                        className="w-full p-4 md:p-6 rounded-xl md:rounded-2xl border-2 border-slate-100 font-bold text-lg md:text-xl focus:border-indigo-500 outline-none bg-slate-50/30"
                        value={q.question}
                        onChange={(e) =>
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      regeneratingQuestionIndex !== qIdx
                    }
                    className="mt-2 sm:mt-0 px-4 py-3 rounded-2xl border border-indigo-200 bg-white text-indigo-600 font-black text-xs hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
                    {regeneratingQuestionIndex === qIdx
                      ? "再生成中…"
                      : "この問を再生成"}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 sm:pl-16 md:pl-24">
                  {q.options.map((opt, oIdx) => (
                    <div
                      key={oIdx}
                      className={`flex items-center gap-3 md:gap-4 p-4 md:p-5 rounded-xl md:rounded-2xl border-2 transition-all ${q.correctAnswer === oIdx ? "border-emerald-500 bg-emerald-50 ring-2 md:ring-4 ring-emerald-100" : "border-slate-100 hover:border-indigo-200"}`}
                    >
                      <input
                        type="radio"
                        checked={q.correctAnswer === oIdx}
                        onChange={() =>
                          updateQuestionField(qIdx, "correctAnswer", oIdx)
                        }
                        className="w-5 h-5 md:w-6 md:h-6 accent-emerald-600 cursor-pointer"
                      />
                      <div className="flex-1">
                        <label className="text-[8px] md:text-[9px] font-black text-slate-300 uppercase block mb-0.5">
                          選択肢 {String.fromCharCode(65 + oIdx)}
                        </label>
                        <input
                          type="text"
                          className="w-full bg-transparent border-none p-0 font-bold text-slate-800 outline-none text-sm md:text-base"
                          value={opt}
                          onChange={(e) =>
                ...
                <div className="sm:pl-16 md:pl-24 space-y-2">
                  ...
                  <textarea
                    className="w-full p-4 md:p-5 rounded-xl md:rounded-2xl border-2 border-slate-100 text-[10px] md:text-sm text-slate-600 bg-slate-50/30 outline-none focus:border-indigo-400 italic font-medium"
                    value={q.explanation}
                    onChange={(e) =>
                      updateQuestionField(qIdx, "explanation", e.target.value)
                    }
                    rows={2}
                  />
                </div>
          ...
          <div className="text-center pt-6 md:pt-10">
            <button
              onClick={handleSaveAllChanges}
              className="w-full max-w-2xl py-6 md:py-8 bg-emerald-600 text-white rounded-2xl md:rounded-[3rem] font-black text-xl md:text-3xl shadow-2xl hover:bg-emerald-700 active:scale-95 transition-all"
            >
              保存・配信を開始
            </button>
          </div>
